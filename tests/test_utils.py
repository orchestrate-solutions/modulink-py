...
from modulink.types import Ctx, Link, ChainFunction, MiddlewareFunction
from modulink.utils import (
    _ensure_async_call, when, catch_errors,
    timing, logging, validate, retry, transform, set_values,
    filter_context, parallel, debounce, memoize, performance_tracker,
    ErrorHandlers, Validators, error_handlers, validators
)
...
def test_utils_imports_and_exports():
    ...
    from modulink.utils import when, parallel, debounce
    assert callable(when)
    assert callable(parallel)
    assert callable(debounce)
    
    # Data transformation
    from modulink.utils import transform, set_values, filter_context
    assert callable(transform)
    assert callable(set_values)
    assert callable(filter_context)
    
    # Middleware functions
    from modulink.utils import catch_errors, timing, logging, validate, retry, memoize
...
async def test_transform_single_link():
    ...
    uppercase_transform = transform("name", lambda value, ctx: value.upper() if value else "")
...
async def test_transform_missing_field():
    ...
    default_transform = transform("missing", lambda value, ctx: "default" if value is None else value)
...
def test_error_handlers_retry_on():
    ...
    retry_handler = ErrorHandlers.retry_on(["ValueError", "ConnectionError"], max_retries=3)
...
async def test_chain_integration_with_utilities():
    ...
    processing_chain = chain(
        ...
        when(lambda ctx: ctx.get("name") == "ALICE", 
             set_values({"special_user": True}))
    )
...
async def test_memoization_and_caching_patterns():
    ...
    memoized_operation = memoize(
        lambda ctx: ctx.get("cache_key", "default"),
        expensive_operation,
        ttl=1.0
    )
...
async def test_complex_utility_composition():
    ...
    data_pipeline = chain(
        ...
        when(lambda ctx: len(ctx.get("data", "")) > 0,
             set_values({"processed": True})),
        
        # Output formatting
        transform("output", lambda _, ctx: {
            "result": ctx.get("data"),
            "timestamp": time.time()
        })
    )
...
async def test_real_world_usage_patterns():
    ...
    api_chain = chain(
        # Authentication
        when(lambda ctx: ctx.get("auth_token"),
             set_values({"authenticated": True})),
        
        # Input validation
        validate(lambda ctx: ctx.get("authenticated", False)),
        
        # Data processing
        transform("user_data", lambda data, ctx: {
