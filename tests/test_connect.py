...
def test_handle_http_connection_missing_params():
    ...
    with pytest.raises(ValueError, match="HTTP connection requires 'app' parameter"):
        _handle_http_connection(mock_modulink, mock_chain_fn, method="GET", path="/test")
...
def test_handle_cron_connection_missing_params():
    ...
    with pytest.raises(ValueError, match="Cron connection requires 'scheduler' parameter"):
        _handle_cron_connection(mock_modulink, mock_chain_fn, cron_expression="0 12 * * *")
    
    # Test missing cron_expression
    with pytest.raises(ValueError, match="Cron connection requires 'cron_expression' parameter"):
...
def test_handle_cli_connection_missing_params():
    ...
    with pytest.raises(ValueError, match="CLI connection requires 'cli_group' parameter"):
        _handle_cli_connection(mock_modulink, mock_chain_fn, command_name="test")
    
    # Test missing command_name
    with pytest.raises(ValueError, match="CLI connection requires 'command_name' parameter"):
...
def test_handle_message_connection_missing_params():
    ...
    # Test missing topic
    with pytest.raises(ValueError, match="Message connection requires 'topic' parameter"):
