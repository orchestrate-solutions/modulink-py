...
async def _ensure_async_call(func: Callable, ctx: Ctx) -> Ctx:
    ...
    try:
        ...
        if not isinstance(result, dict):
            return {**ctx, "error": TypeError(f"Link must return a dictionary, got {type(result)}")}
        ...
        while attempt <= max_attempts:
            try:
                ...
                if attempt < max_attempts:
                    ...
                    # Last attempt failed
                    return {**ctx, "attempts": max_attempts, "error": last_error or ctx.get("error")}
...
class Validators:
    ...
    def required(fields: List[str]):
        ...
        def validator(ctx: Ctx) -> Union[bool, str]:
            missing_fields = [field for field in fields if field not in ctx or ctx[field] is None]
            if missing_fields:
