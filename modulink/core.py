...
from .types import (
    Ctx, Link, ConnectionType, Status,
    ChainFunction, MiddlewareFunction,
    create_context
)
...
def create_modulink(app=None, options: Optional[ModulinkOptions] = None):
        ...
        def _create_chain_handler(self, *links: Link) -> ChainFunction:
            ...
            async def handler(ctx: Ctx) -> Ctx:
                ...
                try:
                    ...
                    result["error"] = e  # Store the actual exception object, not just the string
    ...
    def connect(connection_type, chain_fn, **kwargs):
        ...
        if not isinstance(connection_type, ConnectionType):
            if isinstance(connection_type, str):
                ...
                try:
                    ...
                    raise ValueError(f"Unsupported connection type '{connection_type}'. Supported types: {supported_types}")
            else:
                raise TypeError(f"connection_type must be a ConnectionType enum, got {type(connection_type)}")
        ...
        if connection_type not in CONNECTION_HANDLERS:
            supported_types = [ct.value for ct in ConnectionType]
            raise ValueError(f"Unsupported connection type '{connection_type.value}'. Supported types: {supported_types}")
        ...
        handler_modulink = type('HandlerModulink', (), {
